...
def scanAll(but=None, _and=None):
	myscans = [
	"home":scan.home,
...
if __name__ == '__main__':
	...
	try:args = argv[1:]
		...
		match args[0]:
			case "home":scan.home()
			case "desktop":scan.desktop()
			case "dt":scan.desktop()
			case "pictures":scan.pictures()
			case "pics":scan.pictures()
			case "downloads":scan.downloads()
			case "dl":scan.downloads()
			case "docs":scan.documents()
			case "documents":scan.documents()
			case "templates":scan.templates()
			case "projects":scan.projects()
			case "proj":scan.projects()
			case "music":scan.music()
			case "log":log.read()
			case "all":
				try:
					match args[1]:
						case "but":scanAll(but=args[2])
						case "and":scanAll(_and=args[2])
				except:scanAll()
			case "help":help_me()
			case "-h":help_me()
			case "--help":help_me()
			case "clear":
				try:
					match argv[1]:
						case "log":os.system(f"sudo rm -r {mydirs[0]}/.cleaner_log")
						case "logs":os.system(f"sudo rm -r {mydirs[0]}/.cleaner_log")
				except:print("clear what?")
